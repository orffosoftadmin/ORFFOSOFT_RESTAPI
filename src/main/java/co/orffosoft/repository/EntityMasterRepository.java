...
public interface EntityMasterRepository extends JpaRepository<EntityMaster, Long> {
	...
	@Query("SELECT sm FROM EntityMaster sm WHERE sm.contactNumber=?1")
	EntityMaster findByMobileNo(String number);
	...
	@Query("SELECT em FROM EntityMaster em WHERE em.entityMasterParent.id=:parentId")
	List<EntityMaster> findParentEntityMaster(@Param("parentId") Long parentId);
