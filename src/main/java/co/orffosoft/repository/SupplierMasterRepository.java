...
public interface SupplierMasterRepository extends JpaRepository<SupplierMaster, Long> {
	...
	@Query(nativeQuery = true, value = "select * from supplier_master sm where (sm.code ilike :name or sm.name ilike :name) and sm.user_id =:userId and sm.active_status = true")
	...
	List<SupplierMaster> autoCompletename(@Param("name") String name, @Param("userId") Long userId);
	...
	@Modifying
	@Transactional
	@Query(value = "UPDATE SupplierMaster up SET up.activeStatus=?2  where up.id=?1")
	void updateSupplierStatus(Long headerPk, Boolean inAtive);

	@Query("SELECT sm FROM SupplierMaster sm where sm.code=?1 and sm.createdBy.entityId=?2")
	List<SupplierMaster> checkDuplicateSupplierCode(String lowerCase, Long entityId);

	@Query("SELECT sm FROM SupplierMaster sm where sm.code=?1 And sm.id=?2")
	SupplierMaster findByCodeAndID(String lowerCase, Long id);

	@Query("SELECT sm FROM SupplierMaster sm where sm.name=?1 And sm.id=?2")
	SupplierMaster findByNameAndId(String lowerCase, Long id);

	@Query("SELECT sm FROM SupplierMaster sm where sm.name=?1 and sm.createdBy.entityId=?2")
	List<SupplierMaster> checkDuplicateSupplierName(String lowerCase, Long entityId);
