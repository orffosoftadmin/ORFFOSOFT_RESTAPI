...
public class LoginService {
	...
	public LoginResponseDTO validateUser(LoginDTO loginDTO, HttpServletRequest request) {
		...
		try {
			List<LoginResponseDTO> listLoginResponseDTO = new ArrayList();
			List<LoginResponseDTO> expireditemlist = new ArrayList();
			LoginResponseDTO top_five_product;
				...
				if (userMaster.getStatus()/* && !userMaster.getIsDelete() */) {
					...
					if (password.equals(userMaster.getPassword())) {
						...
						if (entityMaster != null) {
							...
							userMaster.setEntityName(entityMaster.getName());
							userMaster.setEntityCode(entityMaster.getCode());
						}
						...
						Double totalSoldAmt = bill_D_Repository.TodaySoldAmount(sdf.parse(date),
								loginService.getCurrentUser().getEntityId());
						Double totalReturnAmt = salesReturnRepository.TodayReturnAmount(sdf.parse(date),
								loginService.getCurrentUser().getId());
						List<Double> bill = bill_D_Repository.totalbill(sdf.parse(date),
								loginService.getCurrentUser().getEntityId());

						loginResponseDto.setTotalReturnAmt(totalReturnAmt);
						loginResponseDto.setTotalSaleAmount(totalSoldAmt);
						loginResponseDto.setTotalbill(bill.size());
						
						
						
						//for Top Five product
						List<Object[]> product = stockTransactionRepository
								.top_five_product(loginService.getCurrentUser().getEntityId());
						Iterator<Object[]> stockReport = product.iterator();
						while (stockReport.hasNext()) {
							top_five_product = new LoginResponseDTO();
							Object[] ob1 = stockReport.next();

							if (ob1[0] != null) {
								top_five_product.setItemname(ob1[0].toString());

							}
							if (ob1[1] != null) {

								top_five_product.setSoldAmt(Double.parseDouble(ob1[1].toString()));
							}
							listLoginResponseDTO.add(top_five_product);
						}

						loginResponseDto.setListLoginResponseDTO(listLoginResponseDTO);
						
						//for Expired items
						List<Object[]> expireditem = stockTransactionRepository
								.expired_product(loginService.getCurrentUser().getEntityId(), new Date());
						Iterator<Object[]> expireReport = expireditem.iterator();
						
						while (expireReport.hasNext()) {
							top_five_product = new LoginResponseDTO();
							Object[] ob1 = expireReport.next();
							...
							if (ob1[1] != null) {
								...
								top_five_product.setGrndate((Date)ob1[1]);
							}
							if (ob1[2] != null) {

								top_five_product.setExpireddate((Date)ob1[2]);
							}
							expireditemlist.add(top_five_product);
						}
						loginResponseDto.setExpireditemlist(expireditemlist);
	...
	public BaseDTO getDashBoardDetail(HttpServletRequest request) {
		log.info("LoginService >> getDashBoardDetail Method Started ==>>");
		...
		try {
			...
			loginResponseDTO.setTotalSaleAmount(totalSoldAmt);
			loginResponseDTO.setTotalReturnAmt(totalReturnAmt);
			baseDTO.setResponseContent(loginResponseDTO);
			baseDTO.setResponseContents(listLoginResponseDTO);
			baseDTO.setStatusCode(0);
			log.info("LoginService >> getDashBoardDetail Method Ended With StatusCode " + baseDTO.getStatusCode());
		} catch (Exception e) {
			log.error("Exception Occured On LoginService.getDashBoardDetail method == " + e);
		}
	...
	private Map<String, Boolean> loadFeatures(UserMaster userMaster) {
		 ...
		 * 
		 * if (appFeature.getCode() != null) map.put(appFeature.getCode(), true); });
		 * });
		 */

		List<String> featureCodeList = appFeatureRepository.getAppFeatureListByUserName(userMaster.getId());
		if (featureCodeList != null && featureCodeList.size() > 0) {
			for (String str : featureCodeList) {
				map.put(str, true);
			}
	...
	private Map<String, Boolean> loadFeaturesByUserName(UserMaster userMaster) {
		...
		try {
			...
			List<String> featureCodeList = appFeatureRepository.getAppFeatureListByUserName(userMaster.getId());
			// if (userMaster.getRoleType().equalsIgnoreCase("single_user")) {
			// featureCodeList.removeIf(x->x.equals("USER_MASTER_LIST"));
			// }
			if (userMaster.getUserType().equalsIgnoreCase("Cashier")
					|| userMaster.getRoleType().equalsIgnoreCase("")) {
				featureCodeList.removeIf(x -> x.equals("USER_ADD"));
			}
			if (userMaster.getUserType().equalsIgnoreCase("Cashier")) {
				featureCodeList.removeIf(x -> x.equals("PROFIT_LOSS"));
				featureCodeList.removeIf(x -> x.equals("STOCK_UPLOAD_LIST(EXCEL)"));
				featureCodeList.removeIf(x -> x.equals("REPORTS"));
				featureCodeList.removeIf(x -> x.equals("SUPPLIER_MASTER_LIST"));
				featureCodeList.removeIf(x -> x.equals("User Master List"));
				featureCodeList.removeIf(x -> x.equals("STOCK_UPLOAD_LIST"));
				featureCodeList.removeIf(x -> x.equals("CREDIT_NOTE"));
			}
			// if (userMaster.getUserType().equalsIgnoreCase("Cashier")) {
			// featureCodeList.removeIf(x->x.equals("USER_EDIT"));
			// }
	...
	public BaseDTO resetPassword(LoginDTO loginDTO) {
		...
		try {
			...
			if (entityMaster == null || !entityMaster.getCode().equalsIgnoreCase(loginDTO.getStoreCode())) {
			...
			userMaster.setPassword(loginDTO.getPassword());
