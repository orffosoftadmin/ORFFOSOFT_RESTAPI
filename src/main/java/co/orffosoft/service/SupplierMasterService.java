...
public class SupplierMasterService {
	...
	public BaseDTO create(SupplierMaster supplierMaster) {
		...
		List<SupplierMaster> hibList = new ArrayList<SupplierMaster>();
		try {
			Validate.notNull(supplierMaster, ErrorDescription.SUPPLIER_MASTER_SHOULD_NOT_BE_EMPTY);
			Validate.notNullOrEmpty(supplierMaster.getCode(),
					ErrorDescription.SUPPLIER_MASTER_CODE_SHOULD_NOT_BE_EMPTY);
			Validate.notNullOrEmpty(supplierMaster.getName(),
			...
			if (supplierMaster.getCode() != null && !supplierMaster.getCode().trim().equals("")) {
				hibList = new ArrayList<SupplierMaster>();
				hibList = supplierMasterRepository
						.checkDuplicateSupplierCode(supplierMaster.getCode().trim().toLowerCase(), loginService.getCurrentUser().getEntityId());

				if (hibList.size() > 0) {
					log.info("Supplier Code Allready Present");
					response.setMessage("Supplier Code Already Present");
					response.setStatusCode(1);// for check supp code duplicate
			...
			if (supplierMaster.getName() != null && !supplierMaster.getName().trim().equals("")) {
				hibList = new ArrayList<SupplierMaster>();
				hibList = supplierMasterRepository
						.checkDuplicateSupplierName(supplierMaster.getName().trim().toLowerCase(), loginService.getCurrentUser().getEntityId());
				if (hibList.size() > 0) {
					log.info("Supplier Name Allready Present");
					response.setMessage("Supplier Name Already Present");
					response.setStatusCode(2);// for check supp Name duplicate
					throw new RestException("Supplier Name Already Present");
				} 

			}
			supplierMaster.setCode(supplierMaster.getCode().trim().toLowerCase());
			supplierMaster.setName(supplierMaster.getName().trim().toLowerCase());
			...
			supplierMaster.setModifiedDate(new Date());
			supplierMaster.setState(supplierMaster.getState());
			supplierMaster.setCity(supplierMaster.getCity());
			supplierMaster.setAccountholdername(supplierMaster.getAccountholdername());
			supplierMaster.setAccountNumber(supplierMaster.getAccountNumber());
			supplierMaster.setBankname(supplierMaster.getBankname());
			supplierMaster.setIfsccode(supplierMaster.getIfsccode());
	...
	public BaseDTO get(Long id) {
		...
		try {
			...
			if (supplierMaster != null) {
				...
				supplierMasterResponse.setActiveStatus(supplierMaster.getActiveStatus());
				supplierMasterResponse.setState(supplierMaster.getState());
				supplierMasterResponse.setCity(supplierMaster.getCity());
				supplierMasterResponse.setBankname(supplierMaster.getBankname());
				supplierMasterResponse.setAccountNumber(supplierMaster.getAccountNumber());
				supplierMasterResponse.setAccountholdername(supplierMaster.getAccountholdername());
				supplierMasterResponse.setIfsccode(supplierMaster.getIfsccode());
			}
	...
	public BaseDTO update(SupplierMaster supplierMaster) {
		...
		try {
			...
			if (supplierMaster.getCode() != null && !supplierMaster.getCode().trim().equals("")) {
				SupplierMaster supplierMasterCode = supplierMasterRepository
						.findByCodeAndID(supplierMaster.getCode().trim().toLowerCase(), supplierMaster.getId());
				if (supplierMasterCode != null) {
					if (supplierMasterCode.getCode().equals(supplierMaster.getCode())) {
			...
			if (supplierMaster.getName() != null && !supplierMaster.getName().trim().equals("")) {
				SupplierMaster supplierMasterName = supplierMasterRepository
						.findByNameAndId(supplierMaster.getName().trim().toLowerCase(), supplierMaster.getId());

				if (supplierMasterName != null) {
					if (supplierMasterName.getName().equals(supplierMaster.getName())) {
			...
			supMaster.setActiveStatus(supplierMaster.getActiveStatus());
			supMaster.setModifiedDate(new Date());
			supMaster.setModifiedBy(loginService.getCurrentUser());
			
			supMaster.setState(supplierMaster.getState());
			supMaster.setCity(supplierMaster.getCity());
			supMaster.setBankname(supplierMaster.getBankname());
			supMaster.setAccountNumber(supplierMaster.getAccountNumber());
			supMaster.setAccountholdername(supplierMaster.getAccountholdername());
			supMaster.setIfsccode(supplierMaster.getIfsccode());
	...
	public BaseDTO search(SupplierMaster supplierMaster) {
		...
		criteria.createCriteria("supplierMaster.createdBy", "userMaster");
		try {
			...
			projectionList.add(Projections.property("supplierMaster.modifiedDate"));
			projectionList.add(Projections.property("userMaster.username"));
			projectionList.add(Projections.property("supplierMaster.createdBy"));
			...
			if (resultList != null) {
				...
				while (it.hasNext()) {
					...
					responseEntity.setModifiedDate((Date) ob[9]);
					responseEntity.setUserMaster((UserMaster)ob[11]);
...
//		baseDTO.setResponseContent(supplierDetailsDTO);
	...
	public BaseDTO updateSupplierStatus(Long headerPk, String status) {
		log.info("<<======statusInActiveItem in ItemStock Started");
		BaseDTO baseDTO = new BaseDTO();
		try {
			if (status.equals("Active")) {
				Boolean active = true;
				supplierMasterRepository.updateSupplierStatus(headerPk, active);
			} else if (status.equals("In-Active")) {
				Boolean inAtive = false;
				supplierMasterRepository.updateSupplierStatus(headerPk, inAtive);
			}
