...
//		}
	...
	@RequestMapping(value = "/autocompleteitemName/{itemname}/{barcodeType}/{barcode}", method = RequestMethod.GET)
	public ResponseEntity<BaseDTO> autoCompleteItem(@PathVariable("itemname") String itemName ,@PathVariable("barcodeType") String barcodeType ,@PathVariable("barcode") boolean barcode) {
		log.info("generateATNumber Controller Receive ");
		BaseDTO baseDTO = customerBillingService.autoCompleteItemNameWithPrice(itemName,barcodeType,barcode);
		return new ResponseEntity<BaseDTO>(baseDTO, HttpStatus.OK);
	}
	...
	*/
	...
	public @ResponseBody ResponseEntity<BaseDTO> getAllPurchaseOrderDetails(@RequestBody CustomerBillingDTO items) {
		log.info("<--- Received PurchaseOrderItems .create() ---> ");
		BaseDTO baseDTO = new BaseDTO();
