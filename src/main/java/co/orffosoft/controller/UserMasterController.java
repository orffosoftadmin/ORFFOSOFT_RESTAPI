...
public class UserMasterController {
	...
	public BaseDTO update(@RequestBody UserMasterDTO userMasterDto) {

		log.info("<<<< ------- Start UserManagementController.addUserDetails ---------- >>>>>>>");
		return userService.update(userMasterDto);
	...
	@RequestMapping(value = "/updateUserStatus/{headerPk}/{status}", method = RequestMethod.GET)
	public ResponseEntity<BaseDTO> updateUserStatus(@PathVariable Long headerPk, @PathVariable String status) {
		log.info("<---  --->");
		BaseDTO baseDto = userService.updateUserStatus(headerPk, status);
		log.info("<--- End status modify Item CONTROLLER  --->");
		if (baseDto != null) {
	...
	public ResponseEntity<BaseDTO> getShopName(@PathVariable("id") Long id , @PathVariable("pid") Long pid) {
		...
		BaseDTO baseDTO = userService.getShopNames(id,pid);
		return new ResponseEntity<BaseDTO>(baseDTO, HttpStatus.OK);
	}
