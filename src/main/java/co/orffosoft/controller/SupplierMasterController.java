...
public class SupplierMasterController {
	...
	@RequestMapping(value = "/updateSupplierStatus/{headerPk}/{status}", method = RequestMethod.GET)
	public ResponseEntity<BaseDTO> updateItemStatus(@PathVariable Long headerPk, @PathVariable String status) {
		log.info("<--- Starts status modify Item CONTROLLER  --->");
		BaseDTO baseDto = supplierMasterService.updateSupplierStatus(headerPk, status);
